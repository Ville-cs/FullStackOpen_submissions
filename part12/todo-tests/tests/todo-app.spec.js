...
test.describe("Backend API Tests", () => {
  test("GET /api/todos returns todos list", async ({ request }) => {
    const response = await request.get("http://localhost:8080/api/todos");
  ...
  test("GET /api/statistics returns added_todos count from Redis", async ({
    ...
    await request.post("http://localhost:8080/api/todos", {
      data: { text: "Stats test todo", done: false },
    });

    // Then check statistics
    const response = await request.get("http://localhost:8080/api/statistics");
  ...
  test("PUT /api/todos/:id updates a todo", async ({ request }) => {
    ...
    const updateResponse = await request.put(
      `http://localhost:8080/api/todos/${createdTodo._id}`,
      {
